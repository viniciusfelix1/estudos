...
chkconfig --list $SERVIÇO # irá mostrar o status do serviço
chkconfig --level 4,5 $SERVIÇO off # Irá desabilitar o serviço nos runleveis 4 e 5.
...
**SystemD**
O principal diretório do systemd o `/lib/systemd/system/`, neste diretório estão os arquivos de unidade.

O diretório `/etc/systemd/system` tem preferência, ou seja, os arquivos aqui são lidos primeiro.

E por fim, o diretório `/run/systemd/system` geralmente é utilizado de forma temporária.
...
MBR ocupa 512 bytes localizado no primeiro bloco do disco bootavel. Dentro do linux pode ser encontrada como 'msdos'.
