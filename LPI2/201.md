...
`iostat` relatório de estatisticas de CPU e input/output. Ao rodar sem parametros, ele irá mostrar estatisticas desde o ligamento da máquina.
...
`vmstat` é parecido com o iostat, mas mostra infos sobre a memória, io e cpu.
...
`mpstat` trás informações mais detalhadas quanto à CPU.

`sar` Histórico de uso do sistema. Para ele coletar dados em servidores Debian-like, é necessário que ele esteja habilitador no arquivo `/etc/default/sysstat` e o serviço `sysstat` esteja rodando.
