...
Histórico de uso do sistema. Para ele coletar dados em servidores Debian-like, é necessário que ele esteja habilitador no arquivo `/etc/default/sysstat` e o serviço `sysstat` esteja rodando.
...
Coleta o trafego de rede em tempo real e mostra. No menu é possível criar filtros.
...
[*load average*](https://www.helpsystems.com/resources/guides/unix-load-average-part-1-how-it-works) : mostra estatisticas da utilização do processador nos últimos 1, 5 e 15 minutos. É a média de processos executados em relação aos tempos. Ele é alinhado em relação à quantidade de CPUs, por exemplo, em um servidor que possui apenas 1 core, um load de 1.0 é considerado alto por que nos ultimos minutos houve um processo executado a todo momento, enquanto em um servidor com 4 CPUs é considerado normal.
